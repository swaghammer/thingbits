...
def parsePacket(packet):
  ...
  parsedPacket['NodeID'] = splitPacket[1].strip()
  parsedPacket['voltage'] = splitPacket[2].strip()
  parsedPacket['sensorType'] = splitPacket[3].strip()
  parsedPacket['messageType'] = splitPacket[4].strip()
...
def parseBuffer(serialBuffer):
  ...
  if parsedPacket != False:
    if parsedPacket['sensorType'] == 'TMP':
  # if parsedPacket['NodeID'] == 'BOSS':
